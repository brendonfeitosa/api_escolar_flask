...
class ProfessorRepository:
    ...
    def adicionar(self, obj_professor: Professor):
        conn = self.conectar()
        cursor = conn.cursor()
    ...
    def atualizar(self, obj_professor: Professor):
        conn = self.conectar()
        cursor = conn.cursor()

        # Buscar professor antes de atualizar
        ...
        cursor.execute("UPDATE professores SET nome = ?, idade = ?, formacao = ? WHERE id = ?;", (obj_professor.nome, obj_professor.idade, obj_professor.formacao, obj_professor.id))
        conn.commit()
        conn.close()
